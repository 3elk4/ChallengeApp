...
{
    public class CanDeleteRequirement<T> : IAuthorizationRequirement where T : BaseAuditable
    {
    }

    public class CanDeleteHandler<T> : AuthorizationHandler<CanDeleteRequirement<T>> where T : BaseAuditable
    {
        private readonly IDbContext dbContext;
        private readonly IHttpContextAccessor httpContextAccessor;

        public CanDeleteHandler(IDbContext dbContext, IHttpContextAccessor httpContextAccessor)
        {
            this.dbContext = dbContext;
            this.httpContextAccessor = httpContextAccessor;
        }

        protected override Task HandleRequirementAsync(AuthorizationHandlerContext context, CanDeleteRequirement<T> requirement)
        {
